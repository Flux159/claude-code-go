...
export function AppProvider({ children }: AppProviderProps) {
  ...
  useEffect(() => {
    const loadSavedSettings = async () => {
      try {
        ...
        const savedCurrentChatId = await AsyncStorage.getItem(CURRENT_CHAT_ID_STORAGE_KEY);
  ...
  const createNewChat = () => {
    ...
    if (currentChatId && messages.length > 0) {
      try {
        const currentChatIndex = chats.findIndex(c => c && c.id === currentChatId);
        ...
        if (currentChatIndex >= 0) {
          ...
          console.log(`Saved existing conversation with ${messages.length} messages before creating new chat`);
        }
      } catch (error) {
        console.error("Error saving current chat before creating new one:", error);
      }
    }
    ...
    try {
      ...
      if (currentChatId && messages.length > 0) {
        ...
        if (currentChatIndex >= 0) {
          ...
          const chatHasAllMessages = currentChat.messages && 
            messages.every(msg => 
              currentChat.messages.some(chatMsg => chatMsg.id === msg.id)
            );
            
          if (!chatHasAllMessages) {
            console.log(`Saving ${messages.length} messages to current chat before switching`);
      ...
      if (chat) {
        ...
        console.log(`Switched to chat ${chatId} with ${chatMessages.length} messages`);
      } else {
  ...
  useEffect(() => {
    ...
    console.log("Chat initialization - messages array is empty, loading from chat history");
    
    try {
      // Check if we have valid chats data
      const validChats = Array.isArray(chats) && chats.length > 0 && 
      ...
      if (!currentChatId) {
        console.log("No current chat ID but we have chats, selecting most recent");
        // Try to sort the chats by timestamp (newest first)
        try {
          const validChatsArray = chats.filter(chat => chat && typeof chat === 'object');
          ...
          if (mostRecentChat && mostRecentChat.id) {
            ...
            if (messages.length === 0) { // Only set messages if array is empty
              setMessages(Array.isArray(mostRecentChat.messages) ? 
            ...
            if (messages.length === 0) { // Only set messages if array is empty
              setMessages(Array.isArray(firstValidChat.messages) ? 
      ...
      if (messages.length === 0) { // Only create new chat if messages array is empty
        createNewChat();
      }
    }
  ...
  const addMessage = (
    ...
    try {
      ...
      const currentChatIndex = chats.findIndex(chat => chat && chat.id === currentChatId);
      ...
      if (currentChatIndex >= 0) {
        ...
        setChats(updatedChats);
        saveChatsToStorage(updatedChats);
        console.log(`Updated chat ${currentChatId} with new message, now has ${currentChat.messages.length} messages`);
      } else {
